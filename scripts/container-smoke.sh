...
cleanup() {
  ...
  trap - 0 INT TERM
  if [ "$status" -ne 0 ]; then
    printf '%s\n' "container smoke test failed during: $stage" >&2
    docker logs "$collector_name" >&2 2>/dev/null || true
    docker logs "$web_name" >&2 2>/dev/null || true
  fi
...
stage="starting collector container"
docker run --detach \
  --name "$collector_name" \
  --network none \
  --read-only \
...
stage="validating dashboard API and assets"
...
stage="validating container isolation"
...
stage="validating stale-data fallback"
